...
{
	if(state) HAL_GPIO_WritePin(CSN_GPIO_Port, CSN_Pin, GPIO_PIN_SET);
	else HAL_GPIO_WritePin(CSN_GPIO_Port, CSN_Pin, GPIO_PIN_RESET);
}
//2. Chip Enable
void NRF24_ce(int state)
{
	if(state) HAL_GPIO_WritePin(CE_GPIO_Port, CE_Pin, GPIO_PIN_SET);
...
{
  NRF24_begin(CSN_GPIO_Port, CSN_Pin, CE_Pin, nrf24_hspi);
}
...
//4. Init Variables
void nrf24_DebugUART_Init_caller(void)
{
